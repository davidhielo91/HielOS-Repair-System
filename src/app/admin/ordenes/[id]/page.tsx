...
import { ArrowLeft, Save, Copy, Check, CheckCircle, Printer, Plus, Trash2, Clock, MessageSquare, MessageCircle, Wrench, FileText, Send, ThumbsUp, ThumbsDown, ChevronDown } from "lucide-react";
...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    // Fetch the order separately so errors in settings/services don't mask it
    fetch(`/api/orders/${id}`)
      .then((res) => {
      ...
      .catch((e) => {
        setError(`Orden no encontrada (${e.message})`);
        setLoading(false);
      });

    // Fetch the rest independently — failures here won't affect order display
    fetch("/api/settings").then((r) => r.json()).then(setSettings).catch(() => { });
    fetch("/api/services").then((r) => r.json()).then((d) => setAvailableServices(Array.isArray(d) ? d : [])).catch(() => { });
    fetch("/api/parts").then((r) => r.json()).then((d) => setParts(Array.isArray(d) ? d : [])).catch(() => { });
  }, [id]);
  ...
  const handleMarkAsReady = async () => {
    ...
    const updatedOrder = {
      ...
      statusHistory: [
        ...(order.statusHistory || []),
        { from: order.status, to: "listo" as const, date: now, note: "Equipo reparado — listo para entrega" },
      ],
      updatedAt: now,
    };
    try {
      ...
      setOrder({ ...saved, statusHistory: saved.statusHistory || [], internalNotes: saved.internalNotes || [] });
      setSuccess("✅ Equipo marcado como Listo para Entrega");
    ...
    try {
      ...
      const res = await fetch(`/api/orders/${id}`, {
        ...
        body: JSON.stringify({
          ...
          status: "diagnosticando", // Auto-update status
        }),
      });
      if (res.ok) {
        const updated = await res.json();
        setOrder({ ...updated, statusHistory: updated.statusHistory || [], internalNotes: updated.internalNotes || [] });
        setSuccess("Presupuesto enviado y estado actualizado a Diagnosticando");
        setBudgetNote("");
        setTimeout(() => setSuccess(""), 3000);
...
${settings?.termsAndConditions ? settings.termsAndConditions.replace(/\n/g, "<br>") : "Sin términos configurados."}
</div>
        ...
        <div className="flex items-center gap-2">
          {order.status === "reparando" && (
            <button
              ...
              disabled={saving}
              className="flex items-center gap-2 py-2.5 px-4 rounded-lg font-medium text-white bg-emerald-600 hover:bg-emerald-700 transition-colors text-sm shadow-sm disabled:opacity-60"
            >
              <CheckCircle className="h-4 w-4" />
              <span className="hidden sm:inline">Listo para Entrega</span>
